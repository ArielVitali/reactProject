...
function Form() {
  ...
  const { Nombre, Domicilio, Localidad, Telefono, Email, ConfirmarEmail } =
    buyer;
    ...
    return (
      <div className="tiketContenedor">
        <div className="tiket">
          ...
          <p className="compra">
            Te enviamos un mail a <span className="mail">{buyer.Email}</span>
          </p>
          <p className="compra">Tu nro. de orden es:</p>
          <p className="TiketId">{orderId}</p>
          <h2 className="tiketTituloBar">Beer&Burger Resto</h2>
          <Link to={"/"}>
            <p className="carta">Realizar otra compra</p>
          </Link>
        </div>
      </div>
    );
